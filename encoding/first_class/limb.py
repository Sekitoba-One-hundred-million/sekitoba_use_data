...
def main():
    ...
    for k in tqdm( race_data.keys() ):
        ...
        for kk in race_data[k].keys():
            ...
            lib.dic_append( instance_dict, key_limb, { "c": 0, "0": 0, "1": 0, "2": 0 } )
            instance_dict[key_limb]["c"] += 1
            
            if first_horce_body < 4:
                instance_dict[key_limb]["0"] += 1
            elif first_horce_body < 9:
                instance_dict[key_limb]["1"] += 1
            else:
                instance_dict[key_limb]["2"] += 1
                
    result[current_key] = {}
    key_list = [ "0", "1", "2" ]
    
    for limb in instance_dict.keys():
        result[current_key][limb] = {}
        for kk in key_list:
            result[current_key][limb][kk] = instance_dict[limb][kk] / instance_dict[limb]["c"]
            print( "limb:{} class:{} rate:{}".format( limb, kk, result[current_key][limb][kk] ) )
